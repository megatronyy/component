...
public enum TransitionType {
     ...
     * State will not be invoked. An internal Transition can be taken even if the StateMachine is in one or
     * more Regions nested within the associated State.
     */
    INTERNAL,

    /**
     * Impies that the Transition, if triggered, will not exit the composite (source) State, but it
     * will exit and re-enter any state within the composite State that is in the current state configuration.
     */
    LOCAL,

    /**
     * Impies that the Transition, if triggered, will exit the composite (source) State.
     */
    EXTERNAL
}
